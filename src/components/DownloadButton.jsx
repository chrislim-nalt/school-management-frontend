  ...
  return (
    <div className="relative w-full sm:w-auto" ref={ref}>
      <button
        onClick={() => setOpen(o => !o)}
        disabled={!!loading}
        className={`flex items-center justify-center gap-2 px-3 py-2 sm:py-1.5 min-h-[44px] sm:min-h-0 rounded-lg font-semibold text-sm transition-all shadow-sm hover:shadow-md disabled:opacity-60 w-full sm:w-auto ${btnBase}`}
      >
        {loading ? (
          <>
      ...
      {open && (
        <div className="absolute right-0 left-0 sm:left-auto mt-2 w-full sm:w-56 max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-2xl z-50 border border-slate-100 overflow-hidden">
          <div className="px-3 py-2 border-b border-slate-100 bg-slate-50">
            <p className="text-xs font-semibold text-slate-500">Choose format</p>
          </div>
          <button
            onClick={() => handle("pdf")}
            className="w-full flex items-center gap-3 px-3 py-2.5 min-h-[44px] hover:bg-slate-50 active:bg-slate-100 text-left"
          >
